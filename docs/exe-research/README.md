...
| [residential-service-roamer-lifecycle.md](./residential-service-roamer-lifecycle.md) | **Current source of truth** — recovered residential-service scheduler, provider-specific spawn thresholds, finite outbound/return movement, junction visit fields, coverage writes, and independent decay timers; withdraws the inferred ten-tiles-per-Native-day cadence |
...
| [rock-terrain-detail.md](./rock-terrain-detail.md) | Confirmed three rock resource families, 8/4/2 frame shape partition, 3×3 → 2×2 → single-cell rebuild order, subtype mask, and current Native gray-block/footprint gaps |
...
| [housing-evolution-reasons.md](./housing-evolution-reasons.md) | Confirmed `EmperorText` group-127 housing upgrade-reason row matrix, identical `0x51AF60` load path (`group 0x7F`, `reasonCode + 0x27`), and exact target-model reason priority `17…33` from `0x51A660`; row 57 remains the confirmed-safe Native choice, while the row-56 trigger, upstream service/commodity byte writers, and exact `[food_quality]` substitution source remain unknown |
| [desirability-propagation.md](./desirability-propagation.md) | Confirmed original appeal-buffer producer (`0x44F1D0` → `0x44ECD0` → `0x44ED90`), 10-tile clamp, step/step-size updates, 16 angular sectors, and occupied-cell handling; exact offset/occupancy mapping into Native is still unknown, so Qin3 remains fail-closed |
| [population-advisor-housing-capacity.md](./population-advisor-housing-capacity.md) | Confirmed `EmperorText` group-55 rows 8/9/10/12/13/20 alignment; an original screenshot confirms the zero-capacity three-line composition (prefix / gold `0` / suffix) with separate wish + restriction rows. The consumer (`FUN_0053b850` @ `0x53B850`) is recovered. The migration producer is only **partially** recovered (popularity sum, pressure/request/cooldown, assigned/accounted spawn accounting, and the figure-`#11` arrival `house+0x20` write in `FUN_004C9FD0`). Remaining producer blockers: Native mapping of the recovered `FUN_00590F30` food walk (`cMarket+0x2c` @ `0x5437B0` is the recovered normal market-delivery writer/path of `cHouseInfo+0x36`; complete writer set not proven; current Native food consumption/blending/cadence is confirmed non-isomorphic; the live `cStall+0x260` blend/store into `cMarket+0x180` is recovered, mill-pickup cart `+0x13` is the mill `+0x2E4` selected recipe type-count, while model-24 states 6/7 are confirmed to reach `FUN_004EACD0` / `cMarket+0x2C` so that writer is not exclusive to peddlers, and the correct Native representation/mapping remain open, so `lastSuppliedFoodQuality` must not be substituted; see `migration-popularity-producer.md` §3), Native mapping of the recovered `FUN_0055AE30` monument walk (percent / object vector / save lifecycle still unwired; see `migration-popularity-producer.md` §3), and `DAT_01312564` Native war-figure mapping. `DAT_01311FD0` is not a producer input (init-zero and save/load in `migration-popularity-producer.md` §6); its gameplay writer, full value domain, and nonzero-state source remain unknown, so Native nonzero advisor/overlay modes (1 / 2 / other) stay unwired. Native UI still selects only the screenshot-confirmed zero-housing branch |
| [migration-popularity-producer.md](./migration-popularity-producer.md) | Partial recovery: `FUN_00591200` popularity sum/damping, `FUN_005917E0` pressure/request/cooldown, `FUN_004ADA10` assigned/accounted spawn accounting, `FUN_004ADE10` figure `#11` spawn, type-`0xB` dispatch through `FUN_004C7580` to `FUN_004C9FD0` (`house+0x20` add at `0x4CA265`), `house+0x24` lifecycle as a `DAT_01391FE0` land-entry-flood snapshot, house vtable `+0xB8` = `FUN_0042DD40` on `HouseBldg` `0x7ABA38` (`house+9 != 0`), and `+0x1E4` = `FUN_00416B50` returning `cHouseInfo` at `house+0xC8` (`+0x3C != 0` skips the occupancy add). `DAT_01311FD0` is city-stats `DAT_0130F960+0x2670`: three `.text` reads (advisor/overlay; `FUN_0053B850` branches 0 / 1 / 2 / other), `FUN_00590A70` zeros it, `FUN_00593140`/`FUN_0058FE40` persist it; `FUN_005917E0`/`FUN_004AD4A0` and the recovered assignment/arrival chain do not read it, so any value is outside that math. Gameplay writer, full value domain, and nonzero-state source remain unknown; Native nonzero advisor/overlay modes stay unwired. Native occupancy stays fail-closed because Native mapping of the recovered `FUN_00590F30` food walk (`cMarket+0x2c` @ `0x5437B0` is the recovered normal market-delivery writer/path of `cHouseInfo+0x36`; current Native food consumption/blending/cadence is confirmed non-isomorphic; the live `cStall+0x260` blend/store into `cMarket+0x180` is recovered, mill-pickup cart `+0x13` is the mill `+0x2E4` selected recipe type-count, while model-24 states 6/7 are confirmed to reach `FUN_004EACD0` / `cMarket+0x2C` so that writer is not exclusive to peddlers and the correct Native representation/mapping remain open), Native mapping of the recovered `FUN_0055AE30` monument walk, war-figure mapping, Native `+0x24` mapping, and `DAT_00D62408` writer/meaning remain unknown. The `FUN_0055AE30` / `FUN_005604C0` match predicate, index-1 building walk, `cMonumentGoal` `+4==2` / `+0xC` copy, and `FUN_00591200` monument×2 term (matching building-goal pair count, not a distinct-goal count) are recovered in §3 of that note; Native `MonumentProject` / canal / earthen-wall percents are not a proven `FUN_00565410` mapping, so the producer stays `unsupportedOriginalProducer`. `cHouseInfo+0x3C` method identity and occupancy-add skip polarity are closed; original semantic name, complete writer/lifecycle set, and Native mapping are not. House vtable `+0x230` = `FUN_00518DE0` (empty-house args `3`/`0xD` write `house+0x14`/`+0x16` to `(3,0)` / `(13,10)`, mapping to Native `houseLevelID` 0 and 10; original vacant types are building IDs 2 / 11). Original method name, complete caller set, Native representation/lifecycle of IDs 2/11, walker-arrival type-switch timing `2→3` / `11→13` are not; cMarket `+0x268` availability is now closed as a provider-record quantity sum (§10.22), but the provider-to-Native-inventory mapping remains unknown. |
| [qin3-blocker-audit-2026-08-30.md](./qin3-blocker-audit-2026-08-30.md) | Latest player-command replay diagnostic and static evidence boundary for Qin-3 blockers; records the confirmed selector-3 metadata and the remaining unknown entertainment, water, market, and desirability contracts. |
...
7. **Roadblock/market static pass** — `cursor-roadblock-market-pass.md` records
   ...
   and Bbutton ownership stay open. Market shop rows, placement key/capacity
   writes, and dynamic Common/Grand runtime-bay allocation are recorded in
   `migration-popularity-producer.md` §§10.49–10.65; selected-row-to-bay
   mapping, provider quantity, coverage, and settlement remain fail-closed.
...
## Current food-byte correction (2026-08-30)

The migration/food rows above are historical summaries. The current Native
implementation now matches the recovered monthly Dinners draw and preserves
raw `cHouseInfo+0x36` values through delivery and evolution snapshots. The
remaining blockers are specifically the complete market writer/source set,
market-quality producer mapping, and peddler cadence/route equivalence; see
`migration-popularity-producer.md` §§10.18–10.20 for the current contract and
