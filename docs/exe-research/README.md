...
| [migration-popularity-producer.md](./migration-popularity-producer.md) | Partial recovery: `FUN_00591200` popularity sum/damping, `FUN_005917E0` pressure/request/cooldown, `FUN_004ADA10` assigned/accounted spawn accounting, `FUN_004ADE10` figure `#11` spawn, type-`0xB` dispatch through `FUN_004C7580` to `FUN_004C9FD0` (`house+0x20` add at `0x4CA265`), and `house+0x24` lifecycle as a `DAT_01391FE0` land-entry-flood snapshot (`>0` assignment/capacity gate; overlay `rome`, not `roadnet`). Native occupancy stays fail-closed because food fields, monument matching, war-figure mapping, mode writer, Native `+0x24` mapping, and `+0x3C`/`DAT_00D62408` semantics remain unknown |
