...
| [population-advisor-housing-capacity.md](./population-advisor-housing-capacity.md) | Confirmed `EmperorText` group-55 rows 8/9/10/12/13/20 alignment; an original screenshot confirms the zero-capacity three-line composition (prefix / gold `0` / suffix) with separate wish + restriction rows. The consumer (`FUN_0053b850` @ `0x53B850`) and the migration producer chain (popularity `DAT_0130F974` + `FUN_005917E0` pressure bands; calendar case `0x17` `FUN_004AD4A0` accumulates housed newcomers into `DAT_01311FCC` with a monthly copy/reset) are recovered and partly source-mapped. Native UI selects only the screenshot-confirmed zero-housing branch (`availableHousingCapacity < 1` → rows 12/13/20); the row-10 newcomer block and all other unmapped branches fail closed until an equivalent producer is implemented |
