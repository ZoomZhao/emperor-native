...
| [migration-popularity-producer.md](./migration-popularity-producer.md) | Partial recovery: `FUN_00591200` popularity sum/damping, `FUN_005917E0` pressure/request/cooldown, `FUN_004ADA10` assigned/accounted spawn accounting (`house+0x24 > 0` read, semantic mapping unknown), `FUN_004ADE10` figure `#11` spawn, and confirmed type-`0xB` dispatch through `FUN_004C7580` to `FUN_004C9FD0` (states 6→7→8, `house+0x20` add at `0x4CA265`). Native occupancy stays fail-closed because food fields, monument matching, war-figure mapping, mode writer, and house `+0x24`/`+0x3C`/`DAT_00D62408` semantics remain unknown |
