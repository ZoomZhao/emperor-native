...
| [population-advisor-housing-capacity.md](./population-advisor-housing-capacity.md) | Confirmed `EmperorText` group-55 rows 8/9 housing-capacity sentence (`目前住宅还可容纳` + `人居住` / `Housing for` + `more people.`); an original screenshot confirms the zero-capacity three-line composition (prefix / gold `0` / suffix) with separate migration rows, while no draw-call or group-table consumer was recovered from `local/source`, so the executable draw implementation and precise layout constants remain unknown |
