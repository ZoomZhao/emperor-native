...
- **人口顾问住房容量与迁移句：** 住宅顾问的容量行由 `EmperorText` 组 55 第 8/9 行拼装：EN 为 “Housing for” / “more people.”，语义对齐的 GB 18030 简体中文为 `目前住宅还可容纳` / `人居住`，Native 以运行时目录的精确行包住已计算的 `availableHousingCapacity` 并沿用金色数字强调；目录缺失或两行任一为空时整行省略，不合成替代文案。迁移区只使用组 55 第 10/12/13/20 行：10 = `个新移民本月到达`，12 = `移民受到限制.原因是:`（译文保留原版的 ASCII 句点与冒号），13 = `缺乏住房`，20 = `人们希望迁居你的城市`；EN 语义对应 “newcomers arrived this month.” / “Immigration limited by” / “lack of housing vacancies.” / “People wish to come to the city.”。`ClassicTextLocalization.migrationStatusText` 只在这四行运行时全部存在且非空时返回该多元组，无任何硬编码回退。静态恢复的模式 0 渲染器 `FUN_0053b850`（@`0x53B850`，由 `FUN_0053ab80` 人口顾问渲染器到达）给出迁移区的严格优先级：`currentMonthImmigrants > 4` 先画 20 行愿望句，再画独立的新移民块（金色数值计数一行 + 普通色 10 行后缀一行，原版纵向坐标相差 `0x10`）；否则 `availableHousingCapacity < 1` 画愿望句 + 12/13 行限制块。`lastAssessment.blockReason`、`.noEligibleHousing` 与 `plannedImmigrants` 不参与任何玩家可见行的选择（`plannedImmigrants` 不是该渲染器的输入）；旧的“评估存在且 `plannedImmigrants > 0` 时只显示愿望句”路径已删除，原 `migrationStatusRows` 对 `.noEligibleHousing`/评估前容量的依赖同步失效。迁入数 1-4、负国库、高失业、稳定/零迁入及容量为正等其他状态一律不渲染迁移行（fail-closed）。零剩余容量截图（`capcap-260724-160610.png`）确认的可见构成：容量三行（prefix / 金色 0 / suffix）后，先单独一行普通色的迁入意愿 `人们希望迁居你的城市`，再单独一块限制区（引导 `移民受到限制.原因是:` + 金色/警告色原因 `缺乏住房`）；两个操作按钮之后、容量句与迁入意愿之间、以及意愿与限制区之间各有一条水平分隔线。Native 复用按钮后既有的 1px `EmperorTheme.secondary.opacity(0.68)` 细线与 1pt 垂直内边距，不把该 Native 几何表述为已恢复的原版常量。住宅顾问段间构成：容量 / 分隔 / 意愿 / 分隔 / 新移民计数行 + 10 行后缀行（`currentMonthImmigrants > 4`），或容量 / 分隔 / 意愿 / 分隔 / 限制（`availableHousingCapacity < 1`），段间分隔由语义 `AdvisorSummaryLine.separator` 行插入且行尾无多余分隔；其他状态只有容量句也不追加尾部分隔，其他分类不受影响。组 55 双向表格均为 36 行，已直接比对为语义对齐并获目录授权的精确行是 8/9/10/12/13/20；行 11 保持未授权（其迁入数 1 的单数控制依赖尚未源映射的有符号压力等价），行 7/14、其他行与越界一律 fail-closed 返回 nil。原版消费方与状态→文本映射已恢复（`FUN_0053b850` @`0x53B850`），精确字号、色值与超出既有固定面板几何（`control-panel-width` 224px / `panel-content-width` 170px / `population-advisor-height` 280px）之外的新坐标不主张。Native 已验证同构成在 `224px` 面板内不撑宽：Qin-1 UI smoke 在回到住宅分类后、捕获基准图前，以受限遍历递归收集 `advisor-population-panel` 后代的字符串 AX 值/描述/标题并经空白归一化后断言必备 token（`目前住宅还可容纳` `0` `人居住` `人们希望迁居你的城市` `移民受到限制.原因是:` `缺乏住房`）存在、不支持 token（`等待下一个模拟日评估迁入条件` `缺乏临路住房` `国库为负` `失业率过高` `个新移民本月到达`）缺席，再到 AX 框架可得时断言面板宽 `<= 224`；基准态为 `currentMonthImmigrants = 0`、容量为零，因此 10 行后缀必须缺席，防止错误优先级/状态把新人块泄漏进基准图；详见 `docs/exe-research/population-advisor-housing-capacity.md`。
