...
- **住宅服务行人节拍（`inferred`）：** `EmperorFigureModels.txt` 将市场挑夫 `#23` 与水夫 `#28` 的速度都设为 `8`，两者共用普通漫游人物的移动器；Native 已用每天 `10` 个道路格把挑夫的原版 `60` 格行为范围映射到 30 日月历。服务行人必须采用同一每天 `10` 格节拍，不能继续使用无来源的每天 `1` 格近似。此结论由共享模型速度、共享移动链和已校准挑夫月内覆盖交叉支持，但原版逐内部步到 Native 日历的完整换算尚未直接恢复，因此证据等级保持 `inferred`；实现与测试不得把该换算升级为 `confirmed`。
