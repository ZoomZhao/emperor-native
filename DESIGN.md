...
- **人口顾问住房容量句：** 住宅顾问的容量行由 `EmperorText` 组 55 第 8/9 行拼装：EN 为 “Housing for” / “more people.”，语义对齐的 GB 18030 简体中文为 `目前住宅还可容纳` / `人居住`，Native 以运行时目录的精确行包住已计算的 `availableHousingCapacity` 并沿用金色数字强调；目录缺失或两行任一为空时整行省略，不合成替代文案。组 55 双向表格均为 36 行，但只有目标行 8/9 已直接比对为语义对齐（周边 7、10、11 行为支撑上下文），所以目录只授权组 55 的精确行 8/9；与组 127 的整组合对齐不同，组 55 其余行未逐一比对，行查找只放行 8/9，对其他行及越界一律 fail-closed 返回 nil。本实现未从 `local/source` 恢复任何引用该组与两行的绘制调用，因此原版消费方、绘制实现及精确布局常量仍为 `unknown`。原版运行截图（`capcap-260724-160610.png`，2124×1680 物理、1024×768 逻辑内容 2×）已确认剩余容量为零时的可见构成：行 8 前缀单独居中一行、金色 0 单独居中下一行、行 9 后缀居中随下，迁入意愿与限制原因分区保持在其下方独立行；该三行构成、行序与金色数字强调记为 confirmed，但精确字号、色值与超出既有固定面板几何（`control-panel-width` 224px / `panel-content-width` 170px / `population-advisor-height` 280px）之外的新坐标不主张。Native 已验证同三行构成与独立迁移行在 `224px` 面板内不撑宽，详见 `docs/exe-research/population-advisor-housing-capacity.md`。
